#ifndef MANTID_CRYSTAL_MATCHINGPEAKSWORKSPACESTEST_H_
#define MANTID_CRYSTAL_MATCHINGPEAKSWORKSPACESTEST_H_
...
using Mantid::Crystal::MatchingPeaksWorkspaces;

class MatchingPeaksWorkspacesTest : public CxxTest::TestSuite
{
public:
  // This pair of boilerplate methods prevent the suite being created statically
  // This means the constructor isn't called when running other tests
  static MatchingPeaksWorkspacesTest *createSuite() { return new MatchingPeaksWorkspacesTest(); }
  static void destroySuite( MatchingPeaksWorkspacesTest *suite ) { delete suite; }


  void test_init()
  {
    MatchingPeaksWorkspaces alg;
  ...
  // It shouldn't be a problem to match identical workspaces. You just get the same out..
  ...
  void test_no_matches()
  {
    ...
    TS_ASSERT_EQUALS( ws->getPeak(0).getQLabFrame(),lhsWS->getPeak(1).getQLabFrame() )
  ...
  void test_match_peaks_within_tolerance()
  {
    ...
    TS_ASSERT_EQUALS( ws->getPeak(0).getQLabFrame(), lhsWS->getPeak(1).getQLabFrame() )
    TS_ASSERT_EQUALS( ws->getPeak(1).getQLabFrame(), lhsWS->getPeak(3).getQLabFrame() )
    TS_ASSERT_EQUALS( ws->getPeak(2).getQLabFrame(), lhsWS->getPeak(5).getQLabFrame() )
...
#endif /* MANTID_CRYSTAL_MATCHINGPEAKSWORKSPACESTEST_H_ */
