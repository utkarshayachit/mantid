...
namespace Mantid
{
  namespace Algorithms
  {
    ...
    int ReflectometryReductionOneAuto::version() const
    {
      return 1;
    }
    ;
    ...
    void ReflectometryReductionOneAuto::init()
    {
      ...
      declareProperty(
          new WorkspaceProperty<MatrixWorkspace>("InputWorkspace", "", Direction::Input,
              PropertyMode::Mandatory, input_validator), "Input run in TOF");
      ...
      declareProperty(new ArrayProperty<int>("RegionOfDirectBeam", Direction::Input),
      ...
      declareProperty(
          new WorkspaceProperty<MatrixWorkspace>("OutputWorkspaceWavelength", "", Direction::Output),
          "Output workspace in wavelength");

      auto index_bounds = boost::make_shared<BoundedValidator<int> >();
      index_bounds->setLower(0);

      declareProperty(new PropertyWithValue<int>("I0MonitorIndex", Mantid::EMPTY_INT(), index_bounds),
          "I0 monitor index");
      declareProperty(new PropertyWithValue<std::string>("ProcessingInstructions", "", Direction::Input),
          "Processing commands to select and add spectrum to make a detector workspace. See [[PeformIndexOperations]] for syntax.");
      ...
      declareProperty("ThetaOut", Mantid::EMPTY_DBL(), "Calculated final theta in degrees.",
          Direction::Output);
      ...
      declareProperty("StrictSpectrumChecking", true,
          "Strict checking between spectrum numbers in input workspaces and transmission workspaces.");
    ...
    void ReflectometryReductionOneAuto::exec()
    {
      ...
      auto i0_monitor_index = usePropertyValueOrIDF<int>("I0MonitorIndex", instrument,
          "I0MonitorIndex");
      ...
      if (this->getPointerToProperty("ProcessingInstructions")->isDefault())
      {
        if (analysis_mode == "PointDetectorAnalysis")
        {
          processing_commands = boost::lexical_cast<std::string>(
              static_cast<int>(instrument->getNumberParameter("PointDetectorStart")[0])) + ","
              + boost::lexical_cast<std::string>(
                  static_cast<int>(instrument->getNumberParameter("PointDetectorStop")[0]));
        }
        else
        {
          processing_commands = boost::lexical_cast<std::string>(
      ...
      double wavelength_min = usePropertyValueOrIDF<double>("WavelengthMin", instrument, "LambdaMin");
      double wavelength_max = usePropertyValueOrIDF<double>("WavelengthMax", instrument, "LambdaMax");
      auto wavelength_step = isSet<double>("WavelengthStep");
      double wavelength_back_min = usePropertyValueOrIDF<double>("MonitorBackgroundWavelengthMin", instrument,
          "MonitorBackgroundMin");
      double wavelength_back_max = usePropertyValueOrIDF<double>("MonitorBackgroundWavelengthMax", instrument,
          "MonitorBackgroundMax");
      double wavelength_integration_min = usePropertyValueOrIDF<double>("MonitorIntegrationWavelengthMin",
          instrument, "MonitorIntegralMin");
      double wavelength_integration_max = usePropertyValueOrIDF<double>("MonitorIntegrationWavelengthMax",
      ...
      if (!this->getPointerToProperty("PolarizationAnalysis")->isDefault())
      {
        const std::vector<double> crho = this->usePropertyValueOrIDFVec<double>(crhoLabel(), instrument, "crho");
        ...
        refRedOne->setProperty(cppLabel(), cPp);
      }
    ...
    template<typename T>
    T ReflectometryReductionOneAuto::usePropertyValueOrIDF(std::string propName,
        Mantid::Geometry::Instrument_const_sptr instrument, std::string idf_name) const
    {
      ...
      {
        T propValue = this->getProperty(propName);
        return propValue;
      }
    }

    template<typename T>
    std::vector<T> ReflectometryReductionOneAuto::usePropertyValueOrIDFVec(std::string propName,
        Mantid::Geometry::Instrument_const_sptr instrument, std::string idf_name) const
    {
      auto algProperty = this->getPointerToProperty(propName);
      if (algProperty->isDefault())
      ...
      {
        std::vector<T> propValue = this->getProperty(propName);
        return propValue;
      }
    }
