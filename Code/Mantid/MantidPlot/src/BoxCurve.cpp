...
void BoxCurve::drawSeries(QPainter *painter,
		...
		const QRectF &canvasRect, int from, int to) const
{
	...
	drawBoxSymbols(painter, xMap, yMap, dat, size);
...
{
	double x0 = sample(0).x();
	const int px = static_cast<int>(xMap.transform(x0));
	const int px_min = static_cast<int>(xMap.transform(x0 - 0.5));
	const int px_max = static_cast<int>(xMap.transform(x0 + 0.5));
	const int box_width = 1+(px_max - px_min)*b_width/100;
	const int hbw = box_width/2;
	const int median = static_cast<int>(yMap.transform(gsl_stats_median_from_sorted_data (dat, 1, size)));
	...
	if(b_range == SD)
	{
		b_lowerq = static_cast<int>(yMap.transform(mean - sd*b_coeff));
		b_upperq = static_cast<int>(yMap.transform(mean + sd*b_coeff));
	}
	else if(b_range == SE)
	{
		b_lowerq = static_cast<int>(yMap.transform(mean - se*b_coeff));
		b_upperq = static_cast<int>(yMap.transform(mean + se*b_coeff));
	}
	else
	{
		b_lowerq = static_cast<int>(yMap.transform(gsl_stats_quantile_from_sorted_data (dat, 1, size, 1-0.01*b_coeff)));
		b_upperq = static_cast<int>(yMap.transform(gsl_stats_quantile_from_sorted_data (dat, 1, size, 0.01*b_coeff)));
	}
	...
	{
		const int lowerq = static_cast<int>(yMap.transform(gsl_stats_quantile_from_sorted_data (dat, 1, size, 0.25)));
		const int upperq = static_cast<int>(yMap.transform(gsl_stats_quantile_from_sorted_data (dat, 1, size, 0.75)));
		QPolygon pa(8);
	...
	else if (b_style == Notch)
	{
		...
		const int lowerCI = static_cast<int>(yMap.transform(dat[j]));
		const int upperCI = static_cast<int>(yMap.transform(dat[k]));
	...
	{//draw whiskers
		...
		if(w_range == SD)
		{
			w_lowerq = static_cast<int>(yMap.transform(mean - sd*w_coeff));
			w_upperq = static_cast<int>(yMap.transform(mean + sd*w_coeff));
		}
		else if(w_range == SE)
		{
			w_lowerq = static_cast<int>(yMap.transform(mean - se*w_coeff));
			w_upperq =static_cast<int>( yMap.transform(mean + se*w_coeff));
		}
		else
		{
			w_lowerq = static_cast<int>(yMap.transform(gsl_stats_quantile_from_sorted_data (dat, 1, size, 1-0.01*w_coeff)));
			w_upperq =static_cast<int>(yMap.transform(gsl_stats_quantile_from_sorted_data (dat, 1, size, 0.01*w_coeff)));
		}
